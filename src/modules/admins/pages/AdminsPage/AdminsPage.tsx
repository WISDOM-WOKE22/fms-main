"use client";

import { useState, useMemo, useEffect } from "react";
import { useTranslation } from "react-i18next";
import Link from "next/link";
import { useTauriPush } from "@/core/tauri/useTauriPush";
import { DashboardLayout } from "@/core/layout";
import { apiFetch } from "@/core/api/request";
import {
  IconChevronLeft,
  IconChevronRight,
  IconSearch,
  IconPlus,
  IconShield,
} from "@/core/layout/icons";
import { Badge, Select } from "@/core/ui";
import { TableSkeleton } from "@/modules/employees/components/TableSkeleton";
import type { Admin } from "@/modules/admins/types";

const ROWS_PER_PAGE_OPTIONS = [5, 10, 15, 20, 50];

function formatDate(dateStr: string): string {
  const d = new Date(dateStr);
  return d.toLocaleDateString(undefined, { year: "numeric", month: "short", day: "numeric" });
}

function matchesSearch(admin: Admin, query: string): boolean {
  const q = query.trim().toLowerCase();
  if (!q) return true;
  return (
    admin.name.toLowerCase().includes(q) ||
    admin.email.toLowerCase().includes(q) ||
    admin.role.toLowerCase().replace("_", " ").includes(q)
  );
}

export default function AdminsPage() {
  const { t } = useTranslation();
  const push = useTauriPush();
  const [loading, setLoading] = useState(true);
  const [admins, setAdmins] = useState<Admin[]>([]);
  const [page, setPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(15);
  const [searchQuery, setSearchQuery] = useState("");

  useEffect(() => {
    let mounted = true;
    setLoading(true);
    apiFetch("/api/v1/admins")
      .then((res) => res.json())
      .then((data: Admin[]) => {
        if (mounted && Array.isArray(data)) setAdmins(data);
      })
      .catch(() => {
        if (mounted) setAdmins([]);
      })
      .finally(() => {
        if (mounted) setLoading(false);
      });
    return () => { mounted = false; };
  }, []);

  const filteredAdmins = useMemo(
    () => admins.filter((a) => matchesSearch(a, searchQuery)),
    [admins, searchQuery]
  );

  const totalPages = Math.max(1, Math.ceil(filteredAdmins.length / rowsPerPage));
  const start = (page - 1) * rowsPerPage;
  const paginatedRows = useMemo(
    () => filteredAdmins.slice(start, start + rowsPerPage),
    [filteredAdmins, start, rowsPerPage]
  );

  const goPrev = () => setPage((p) => Math.max(1, p - 1));
  const goNext = () => setPage((p) => Math.min(totalPages, p + 1));

  return (
    <DashboardLayout title={t("nav.admins")}>
      <div className="w-full max-w-none">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <p className="text-sm text-fms-text-secondary m-0">{t("admins.pageDescription")}</p>
          <Link
            href="/admins/new"
            className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-2xl text-sm font-semibold text-white bg-fms-accent hover:bg-fms-accent-hover shadow-sm hover:shadow-md transition-all duration-300 ease-out focus:outline-none focus:ring-2 focus:ring-fms-accent-muted focus:ring-offset-2"
            aria-label={t("admins.addAdmin")}
          >
            <IconPlus className="w-5 h-5" aria-hidden />
            {t("admins.addAdmin")}
          </Link>
        </div>

        <section className="rounded-2xl bg-fms-surface border border-fms-border overflow-hidden shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 border-b border-fms-border bg-fms-bg-subtle/20">
            <div className="relative flex-1 max-w-sm">
              <IconSearch
                className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-fms-text-tertiary pointer-events-none"
                aria-hidden
              />
              <input
                type="search"
                placeholder={t("admins.searchPlaceholder")}
                value={searchQuery}
                onChange={(e) => {
                  setSearchQuery(e.target.value);
                  setPage(1);
                }}
                className="w-full pl-9 pr-4 py-2.5 rounded-2xl text-sm text-fms-text bg-fms-surface border border-fms-border placeholder:text-fms-text-tertiary focus:outline-none focus:ring-2 focus:ring-fms-accent-muted focus:border-fms-accent transition-shadow"
                aria-label={t("admins.searchPlaceholder")}
              />
            </div>
          </div>

          <div className="overflow-x-auto">
            {loading ? (
              <TableSkeleton rows={rowsPerPage} cols={7} showCheckbox={false} />
            ) : paginatedRows.length === 0 ? (
              <div className="flex flex-col items-center justify-center px-4 py-16 text-center">
                <div className="rounded-2xl bg-fms-bg-subtle p-4 mb-4 text-fms-text-tertiary">
                  <IconShield className="w-10 h-10" aria-hidden />
                </div>
                <p className="text-sm text-fms-text-secondary m-0">{t("admins.noResults")}</p>
              </div>
            ) : (
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="border-b border-fms-border bg-fms-bg-subtle/30">
                    <th className="py-3.5 px-4 font-medium text-fms-text-tertiary text-left">
                      {t("admins.adminName")}
                    </th>
                    <th className="py-3.5 px-4 font-medium text-fms-text-tertiary text-left">
                      {t("admins.email")}
                    </th>
                    <th className="py-3.5 px-4 font-medium text-fms-text-tertiary text-left">
                      {t("admins.role")}
                    </th>
                    <th className="py-3.5 px-4 font-medium text-fms-text-tertiary text-left">
                      {t("admins.status")}
                    </th>
                    <th className="py-3.5 px-4 font-medium text-fms-text-tertiary text-left">
                      {t("admins.permissions")}
                    </th>
                    <th className="py-3.5 px-4 font-medium text-fms-text-tertiary text-left">
                      {t("admins.createdAt")}
                    </th>
                    <th className="py-3.5 px-4 font-medium text-fms-text-tertiary text-left w-28">
                      {t("admins.actions")}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {paginatedRows.map((row) => (
                    <tr
                      key={row.id}
                      role="button"
                      tabIndex={0}
                      onClick={() => push(`/admins/${row.id}`)}
                      onKeyDown={(e) => {
                        if (e.key === "Enter" || e.key === " ") {
                          e.preventDefault();
                          push(`/admins/${row.id}`);
                        }
                      }}
                      className="border-b border-fms-border last:border-b-0 hover:bg-fms-bg-subtle/50 transition-colors cursor-pointer"
                      aria-label={`${t("admins.view")} ${row.name}`}
                    >
                      <td className="py-3.5 px-4 font-medium text-fms-text">
                        {row.name}
                      </td>
                      <td className="py-3.5 px-4 text-fms-text-secondary">
                        {row.email}
                      </td>
                      <td className="py-3.5 px-4">
                        <Badge variant={row.role === "super_admin" ? "warning" : "neutral"}>
                          {row.role === "super_admin"
                            ? t("admins.superAdmin")
                            : t("admins.subAdmin")}
                        </Badge>
                      </td>
                      <td className="py-3.5 px-4">
                        <Badge variant={row.status === "active" ? "success" : "danger"}>
                          {row.status === "active"
                            ? t("admins.active")
                            : t("admins.inactive")}
                        </Badge>
                      </td>
                      <td className="py-3.5 px-4 text-fms-text-secondary">
                        {row.role === "super_admin" ? (
                          <span className="text-fms-text-tertiary">{t("admins.fullAccess")}</span>
                        ) : (
                          <span>{row.permissions.length} {t("admins.modules")}</span>
                        )}
                      </td>
                      <td className="py-3.5 px-4 text-fms-text-secondary tabular-nums">
                        {formatDate(row.createdAt)}
                      </td>
                      <td className="py-3.5 px-4" onClick={(e) => e.stopPropagation()}>
                        <Link
                          href={`/admins/${row.id}`}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-2xl text-sm font-medium border border-fms-border text-fms-text hover:bg-fms-bg-subtle hover:border-fms-accent/40 transition-colors"
                          aria-label={`${t("admins.view")} ${row.name}`}
                        >
                          {t("admins.view")}
                        </Link>
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            )}
          </div>

          {!loading && filteredAdmins.length > 0 && (
            <div className="flex flex-wrap items-center justify-between gap-4 px-4 py-3 border-t border-fms-border bg-fms-bg-subtle/20">
              <div className="flex flex-wrap items-center gap-4">
                <div className="flex items-center gap-2">
                  <label className="text-sm text-fms-text-secondary whitespace-nowrap">{t("admins.rowsPerPage")}</label>
                  <Select
                    value={rowsPerPage}
                    onChange={(e) => {
                      setRowsPerPage(Number(e.target.value));
                      setPage(1);
                    }}
                    selectSize="sm"
                    className="min-w-[72px]"
                    aria-label={t("admins.rowsPerPage")}
                  >
                    {ROWS_PER_PAGE_OPTIONS.map((n) => (
                      <option key={n} value={n}>
                        {n}
                      </option>
                    ))}
                  </Select>
                </div>
                <span className="text-sm text-fms-text-secondary">
                  {t("admins.pageOf", { current: page, total: totalPages })}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={goPrev}
                  disabled={page <= 1}
                  className="inline-flex items-center gap-1 px-3 py-1.5 rounded-2xl text-sm font-medium text-fms-text-secondary hover:text-fms-text hover:bg-fms-bg-subtle disabled:opacity-50 disabled:pointer-events-none transition-colors"
                >
                  <IconChevronLeft className="w-4 h-4" />
                  {t("admins.previous")}
                </button>
                <button
                  type="button"
                  onClick={goNext}
                  disabled={page >= totalPages}
                  className="inline-flex items-center gap-1 px-3 py-1.5 rounded-2xl text-sm font-medium text-fms-text-secondary hover:text-fms-text hover:bg-fms-bg-subtle disabled:opacity-50 disabled:pointer-events-none transition-colors"
                >
                  {t("admins.next")}
                  <IconChevronRight className="w-4 h-4" />
                </button>
              </div>
            </div>
          )}
        </section>
      </div>
    </DashboardLayout>
  );
}
